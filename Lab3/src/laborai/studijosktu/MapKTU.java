...
public class MapKTU<K, V> implements MapADTp<K, V> {
    ...
    public boolean containsValue(V value) {
        if (value == null) {
            throw new IllegalArgumentException("Value is null in containsValue(Value value)");
        }
        
        for(int i = 0; i < table.length; i++) {
            if(containsValueInChain(value, table[i]))
                return true;
        }
        return false;
    }
    
    private boolean containsValueInChain(V value, Node node) {
        if (value == null) {
            throw new IllegalArgumentException("Value or node is null in cotainsValueInChain(Value value, Node node)");
        }
    ...
    public int getAvarageChainLenght() {
        ...
        for(int i = 0; i < table.length; i++) {
            avgLen += getChainLenght(table[i]);
        }
        
        return avgLen / table.length;
    }
    
    private int getChainLenght(Node node) {
        int chainSize = 0;
        for (Node<K, V> n = node; n != null; n = n.next)
            chainSize++;
        return chainSize;
    ...
    /**
     * Grąžina maišos lentelės masyvo tusčiųjnj.
     * 
     * @return vidutinis grandinės ilgis.
     */
    @Override
    ...
    public List<V> chainValues(Node node) {
        List<V> result = new ArrayList<V>();
        for (Node<K, V> n = node; n != null; n = n.next)
            result.add(n.value);
        return result;
