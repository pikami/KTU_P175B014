...
public class ListKTU<E extends Comparable<E>>
	...
	public E set(int k, E e) {
            ...
            current = first.findNode(k);
            E previousValue = current.element;
            current.element = e;
	...
	public E remove(int k) {
            ...
            if (k == 0) {
                ...
                Node penult = first.findNode(k - 1);
                penult.next = null;
                last = penult;
            } else {
        ...
        List<E> subList(int fromIndex, int toIndex) {
            try {
                ...
                for (int i = fromIndex; i<toIndex; i++){
                    Object clone = current.element.getClass().newInstance();
                    for (Field field : current.element.getClass().getDeclaredFields()) {
                        field.setAccessible(true);
                        field.set(clone, field.get(current.element));
                    }
                    result.add((E)clone);
                    current = current.next;
                }
                ...
                Logger.getLogger(ListKTU.class.getName()).log(Level.SEVERE, null, ex);
